    ...
    async startChat(extraHistory) {
        ...
        try {
            ...
            const generateContentConfigWithThinking = isThinkingSupported(this.config.getModel())
                ...
                : this.generateContentConfig;
            return new OmniChat(this.config, this.getContentGenerator(), {
                systemInstruction,
    ...
    async *sendMessageStream(request, signal, prompt_id, turns = this.MAX_TURNS, originalModel) {
        ...
        if (this.config.getIdeMode()) {
            ...
            if (openFiles) {
                ...
                if (openFiles.activeFile) {
                    contextParts.push(`This is the file that the user was most recently looking at:\n- Path: ${openFiles.activeFile}`);
                    if (openFiles.cursor) {
                    ...
                    if (openFiles.selectedText) {
                        contextParts.push(`This is the selected text in the active file:\n- ${openFiles.selectedText}`);
                    }
                }
        ...
        if (!turn.pendingToolCalls.length && signal && !signal.aborted) {
            ...
            if (nextSpeakerCheck?.next_speaker === 'model') {
                logFlashDecidedToContinue(this.config, new FlashDecidedToContinueEvent(prompt_id));
