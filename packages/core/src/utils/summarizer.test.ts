...
describe('summarizers', () => {
  ...
  beforeEach(() => {
    ...
    (mockModelClient.generateContent as Mock) = vi.fn();
  ...
  describe('summarizeToolOutput', () => {
    it('should return original text if it is shorter than maxLength', async () => {
      ...
      expect(mockModelClient.generateContent).not.toHaveBeenCalled();
    });
    ...
    it('should call generateContent if text is longer than maxLength', async () => {
      ...
      (mockModelClient.generateContent as Mock).mockResolvedValue({
      ...
      expect(mockModelClient.generateContent).toHaveBeenCalledTimes(1);
    ...
    it('should return original text if generateContent throws an error', async () => {
      ...
      (mockModelClient.generateContent as Mock).mockRejectedValue(error);
    ...
    it('should construct the correct prompt for summarization', async () => {
      ...
      await summarizeToolOutput(longText, mockModelClient, abortSignal, 1000);
...
`;
      const calledWith = (mockModelClient.generateContent as Mock).mock
        .calls[0];
