...
vi.mock('../core/client.js', () => ({
  ModelClient: vi.fn().mockImplementation(function (
    this: any,
    _config: Config,
  ) {
...
describe('editCorrector', () => {
  ...
  describe('ensureCorrectEdit', () => {
    ...
    beforeEach(() => {
      ...
      mockModelClientInstance = new ModelClient(
        mockConfigInstance,
      ) as Mocked<ModelClient>;
      mockModelClientInstance.getHistory = vi.fn().mockResolvedValue([]);
      resetEditCorrectorCaches_TEST_ONLY();
    });
    ...
    describe('Scenario Group 6: Concurrent Edits', () => {
      it('Test 6.1: should return early if file was modified by another process', async () => {
        ...
        (mockModelClientInstance.getHistory as Mock).mockResolvedValue(
          history,
        );
