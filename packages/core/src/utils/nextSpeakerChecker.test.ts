...
// Mock ModelClient and Config constructor
...
describe('checkNextSpeaker', () => {
  ...
  beforeEach(() => {
    ...
    mockModelClient = new ModelClient(mockConfigInstance);
  ...
  it('should return null if history is empty', async () => {
    ...
    expect(mockModelClient.generateJson).not.toHaveBeenCalled();
  });
  ...
  it("should return { next_speaker: 'model' } when model intends to continue", async () => {
    ...
    (mockModelClient.generateJson as Mock).mockResolvedValue(mockApiResponse);
    ...
    expect(mockModelClient.generateJson).toHaveBeenCalledTimes(1);
  });
  ...
  it('should return null if geminiClient.generateJson throws an error', async () => {
    ...
    (mockModelClient.generateJson as Mock).mockRejectedValue(
      new Error('API Error'),
    );
  ...
  it('should return null if geminiClient.generateJson returns invalid JSON (missing next_speaker)', async () => {
    ...
    (mockModelClient.generateJson as Mock).mockResolvedValue({
  ...
  it('should call generateJson with DEFAULT_GEMINI_FLASH_MODEL', async () => {
    ...
    await checkNextSpeaker(chatInstance, mockModelClient, abortSignal);

    expect(mockModelClient.generateJson).toHaveBeenCalled();
    const generateJsonCall = (mockModelClient.generateJson as Mock).mock
      .calls[0];
