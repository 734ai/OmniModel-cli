...
import { findIndexAfterFraction, ModelClient } from './client.js';
...
describe('Gemini Client (client.ts)', () => {
  ...
  beforeEach(async () => {
    ...
    // Because the ModelClient constructor kicks off an async process (startChat)
    ...
    client = new ModelClient(new Config({} as never));
    mockConfigObject.getModelClient.mockReturnValue(client);
  ...
  // was not being detected as called by the ModelClient instance.
  // This likely points to a subtle issue in how the GoogleGenerativeAI class constructor
  // and its instance methods are mocked and then used by the class under test.
  // For future debugging, ensure that the `this.client` in `ModelClient` (which is an
  ...
  // For future debugging, ensure `this.client.models.generateContent` in `ModelClient` correctly
  ...
  describe('sendMessageStream', () => {
    it('should include IDE context when ideMode is enabled', async () => {
      ...
      const mockChat: Partial<OmniChat> = {
        addHistory: vi.fn(),
        getHistory: vi.fn().mockReturnValue([]),
      };
      client['chat'] = mockChat as OmniChat;
