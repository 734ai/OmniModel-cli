...
import { Config, FileDiscoveryService } from '@omnimodel/cli-core';
...
// Mock dependencies
vi.mock('fs/promises');
vi.mock('@omnimodel/cli-core', async () => {
  const actual = await vi.importActual('@omnimodel/cli-core');
  return {
    ...actual,
...
describe('useCompletion git-aware filtering integration', () => {
  ...
  beforeEach(() => {
    mockFileDiscoveryService = {
      ...
      shouldOmniIgnoreFile: vi.fn(),
      shouldIgnoreFile: vi.fn(),
      filterFiles: vi.fn(),
      getOmniIgnorePatterns: vi.fn(),
      projectRoot: '',
    ...
    mockFileDiscoveryService.shouldIgnoreFile.mockImplementation(
      (path: string, options) => {
        ...
        if (
          ...
          mockFileDiscoveryService.shouldOmniIgnoreFile
        ) {
          return mockFileDiscoveryService.shouldOmniIgnoreFile(path);
        }
        return false;
      },
    );
