...
import { ApprovalMode, Config, EditTool, ShellTool, WriteFileTool, sessionId, logUserPrompt, AuthType, getOauthClient, } from '@omnimodel/cli-core';
...
function getNodeMemoryArgs(config) {
    ...
    if (process.env.OMNIFORM_CLI_NO_RELAUNCH) {
        return [];
    }
...
async function relaunchWithAdditionalArgs(additionalArgs) {
    ...
    const newEnv = { ...process.env, OMNIFORM_CLI_NO_RELAUNCH: 'true' };
...
export async function main() {
    ...
    if (settings.merged.selectedAuthType === AuthType.LOGIN_WITH_OAUTH &&
...
function setWindowTitle(title, settings) {
    if (!settings.merged.hideWindowTitle) {
        const windowTitle = (process.env.CLI_TITLE || `OmniForm - ${title}`).replace(
