...
import { Box, DOMElement, measureElement, Static, Text, useStdin, useStdout, useInput, } from 'ink';
...
import { getErrorMessage, getAllGeminiMdFilenames, ApprovalMode, isEditorAvailable, EditorType, FlashFallbackEvent, logFlashFallback, AuthType, ideContext, } from '@google/gemini-cli-core';
...
const App = ({ config, settings, startupWarnings = [], version }) => {
    ...
    useEffect(() => {
        ...
        if (!isAuthenticating) {
            setUserTier(config.getModelClient()?.getUserTier());
        }
...
⚡ You can switch authentication methods by typing /auth`;
    ...
    const { streamingState, submitQuery, initError, pendingHistoryItems: pendingGeminiHistoryItems, thought, } = useGeminiStream(config.getModelClient(), history, addItem, setShowHelp, config, setDebugMessage, handleSlashCommand, shellModeActive, getPreferredEditor, onAuthError, performMemoryRefresh, modelSwitchedFromQuotaError, setModelSwitchedFromQuotaError);
    ...
    const geminiClient = config.getModelClient();
    useEffect(() => {
        if (initialPrompt &&
